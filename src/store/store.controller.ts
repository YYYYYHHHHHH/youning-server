...
export class StoreController {
  ...
  @ApiResponse({ 
    status: 200, 
    description: '成功获取仓库列表。每个仓库对象都包含其关联的项目信息（如果有）。' 
  })
  ...
  @ApiOperation({ 
    summary: '创建仓库',
    description: '创建一个新的仓库。每个仓库可以关联一个项目，并且这种关联是一对一的关系（一个仓库只能关联一个项目，一个项目也只能关联一个仓库）。'
  })
  @ApiResponse({ 
    status: 201, 
    description: '成功创建仓库。返回的数据中包含关联的项目信息（如果有）。' 
  })
  ...
  @ApiOperation({ 
    summary: '更新仓库信息',
    description: '更新仓库信息，包括仓库名称和项目关联关系。注意：修改项目关联关系时需确保目标项目未被其他仓库关联。'
  })
  @ApiResponse({ 
    status: 200, 
    description: '成功更新仓库信息。返回更新后的完整仓库信息，包含关联的项目详情。' 
  })
