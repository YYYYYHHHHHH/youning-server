...
核心业务模块，负责管理施工工地相关信息。在施工项目表中包含`salesProjectId`字段，用于关联销售项目表，实现销售项目与施工工地的关联。

#### 销售项目与施工工地关联功能

- 施工项目(Project)表中的`salesProjectId`字段用于关联销售项目(SalesProject)表
- API接口`GET /projects/pending/construction`用于查询跟进状态为"成交"但尚未创建施工工地的销售项目
- 此功能可用于在管理中心显示需要创建施工工地的通知

**使用方法**：
1. 创建或更新施工项目时，可以设置`salesProjectId`字段关联到对应的销售项目
2. 调用`GET /projects/pending/construction`接口获取需要创建施工工地的销售项目列表
3. 前端可以根据返回的销售项目列表，在管理中心显示相应的通知
